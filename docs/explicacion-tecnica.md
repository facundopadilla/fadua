...
Esos módulos se ejecutan de dos formas. En el VPS corren orquestados por Prefect: un flow que llama a cada adaptador como una task, con un schedule de 5 minutos y una UI donde se ve cada corrida, sus logs y sus reintentos. El mismo código se puede correr como cron plano (`python -m sync`), que queda en el repo como referencia sin dependencias extra. Hay un detalle de seguridad en el flow: cada task carga las credenciales desde el entorno en vez de recibirlas como argumento, porque Prefect registra los argumentos de las tasks y no quiero que un secreto termine visible en la UI. Entre tasks solo viajan los datos de los productos, que son públicos.
