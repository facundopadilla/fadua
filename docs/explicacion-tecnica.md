...
Esa lógica vive en dos carpetas autónomas, una por cada forma de ejecutarla. En `python/` corre como cron plano (`python -m sync`), sin dependencias extra. En `prefect/` corre orquestada con Prefect: un flow que llama a cada adaptador como una task, con un schedule de 5 minutos y una UI donde se ve cada corrida, sus logs y sus reintentos. Cada carpeta trae su propia copia de los módulos, así que se levanta sola. Hay un detalle de seguridad en el flow: cada task carga las credenciales desde el entorno en vez de recibirlas como argumento, porque Prefect registra los argumentos de las tasks y no quiero que un secreto termine visible en la UI. Entre tasks solo viajan los datos de los productos, que son públicos.
